...
typedef
struct NodeDTTCover_
{
   cType dep;
   long dv;
   struct NodeDTTCover_ * next;
} NodeDTTCover;
...
struct DepRange_
{
   cType dep1;
   cType dep2;
   struct DepRange_ * next;
} DepRange;


typedef
struct AnsAdjustRec_
{
   cType n;
   struct AnsAdjustRec_ * next;
   struct AnsAdjustRec_ * prev;
...
struct NodePropArr_{
   ...
   NodeDTTCover *freePndc;
   long idxFreePndc;
   NodeDTTCover **perNodeheadPndc;
   NodeDTTCover **perNodeheadPndcSign0;

   AnsAdjustRec *freeAnsAdjustRec;
   long idxAnsAdjustRec;
   AnsAdjustRec * pCh1;
   AnsAdjustRec * p2Ch;
   DepRange * pRch;
   aType * pSadj1;
...
struct PreprocData_{
  ...
  //<BEGIN> for anylevel
   ...
   NodeDTTCover *gFreePndc;
   long gIdxFreePndc;
   NodeDTTCover **gperNodeheadPndc;
   NodeDTTCover **gperNodeheadPndcSign0;

   AnsAdjustRec *gFreeAnsAdjustRec;
   long gIdxAnsAdjustRec;
   AnsAdjustRec *gpCh1;
   AnsAdjustRec *gp2Ch;
   DepRange *gpRch;

   aType *gpSadj1; //运算中补回标记，这个祖先是否被补回
   aType *gpSadj2; //运算中补回标记，这个祖先是否被补回
  //<END>
  ...
  int LEVEL_NUM;
  int K; //the upper bound of edge connectivity
